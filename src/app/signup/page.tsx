...
const schema: ZodSchema<FormData> = z.object({
    ...
    date_of_birth: z.date().refine(date => date <= new Date(), 'Date of Birth must be in the past'), // Changed to z.date()
...
const SignUpPage: React.FC = () => {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      const response = await axios.post('http://localhost:8080/api/v1/access/register', data);
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="mb-4">
            ...
            <input
              ...
              className={`w-full px-3 py-2 border rounded ${errors.date_of_birth ? 'border-red-500' : ''}`}
            />
